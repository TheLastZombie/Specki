				...
				function getmap(i) {
					if (tbest[i]) {
						request("https://osu.ppy.sh/api/get_beatmaps?k=" + process.env.OSU_TK + "&b=" + JSON.parse(body)[i].beatmap_id, function (error, reponse, body) {
							best += "[" + JSON.parse(body)[0].artist + " – " + JSON.parse(body)[0].title + "](https://osu.ppy.sh/beatmaps/" + JSON.parse(body)[0].beatmap_id + ") (Rank: " + tbest[i].rank + " · " + tbest[i].pp + " PP · Score: " + tbest[i].score + ")\n";
							getmap(i + 1);
						});
					} else {
						var recent = "";
						request("https://osu.ppy.sh/api/get_user_recent?k=" + process.env.OSU_TK + "&u=" + args.join(" ") + "&limit=5", function (error, response, body) {
							...
							function getmapr(i) {
								if (trecent[i]) {
									request("https://osu.ppy.sh/api/get_beatmaps?k=" + process.env.OSU_TK + "&b=" + JSON.parse(body)[i].beatmap_id, function (error, reponse, body) {
										recent += "[" + JSON.parse(body)[0].artist + " – " + JSON.parse(body)[0].title + "](https://osu.ppy.sh/beatmaps/" + JSON.parse(body)[0].beatmap_id + ") (Rank: " + trecent[i].rank + " · Score: " + trecent[i].score + ")\n";
										getmapr(i + 1);
									});
								} else {
